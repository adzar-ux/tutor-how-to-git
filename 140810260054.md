...
2. Leave the default settings and click **Create fork**.
...
2. Make a new `student_<NPM-mu>/` folder, create a new file named `<NPM-mu>.md` (replace `your-github-username` with your actual GitHub username).
3. Inside that file, write a quick greeting, like this:
...
1. Go back to your terminal and check the status of your project:
   ```bash
   git status
   ```
2. **Add (Stage)** your new file so Git starts tracking it:
   ```bash
   git add .
   ```
3. **Commit** your file to save a snapshot in your local history:
   ```bash
...
1. Open your web browser and navigate to **your forked repository** (`github.com/YOUR-USERNAME/tutor-how-to-git`).
2. Refresh the page.
3. Navigate into your folder and verify that your new `<NPM-mu>.md` file is present!
